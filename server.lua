...
local CLIPBOARD_DEBUG_FILE = "clipboard_debug.txt"
local CLIPBOARD_BOX_EMPTY = "\226\152\144"
local CLIPBOARD_BOX_CHECKED = "\226\152\145"
local CLIPBOARD_CHECK_MARK = "\226\156\147"
local CLIPBOARD_HEAVY_CHECK_MARK = "\226\156\148"
...
local function cleanClipboardText(s)
    s = tostring(s or "")
    s = s:gsub("\r", " "):gsub("\n", " ")
    s = s:gsub("%s+", " ")
...
local function normalizeClipboardLabel(s)
    s = cleanClipboardText(s):lower()
    s = s:gsub("%[[xX ]%]", " ")
    s = s:gsub("%([xX ]%)", " ")
    s = s:gsub(CLIPBOARD_BOX_EMPTY, " ")
    s = s:gsub(CLIPBOARD_BOX_CHECKED, " ")
    s = s:gsub(CLIPBOARD_CHECK_MARK, " ")
    s = s:gsub(CLIPBOARD_HEAVY_CHECK_MARK, " ")
    s = s:gsub("^%s*[%-%*]+%s*", "")
    s = s:gsub("^x%d+%s+", "")
    s = s:gsub("^%d+%s*[xX]%s+", "")
    s = s:gsub("%s+[xX]%s*%d+%s*$", "")
...
local function clipboardLineState(line)
    local lower = cleanClipboardText(line):lower()
    if lower:find("%[[xX]%]") or lower:find("%([xX]%)")
            or lower:find(CLIPBOARD_BOX_CHECKED, 1, true)
            or lower:find(CLIPBOARD_CHECK_MARK, 1, true)
            or lower:find(CLIPBOARD_HEAVY_CHECK_MARK, 1, true)
            or lower:find("checked%s*[:=]%s*true")
            or lower:find("done%s*[:=]%s*true")
            or lower:find("complete%s*[:=]%s*true") then
        return "checked"
    end
    if lower:find("%[%s%]") or lower:find("%(%s%)")
            or lower:find(CLIPBOARD_BOX_EMPTY, 1, true)
            or lower:find("unchecked")
            or lower:find("missing")
            or lower:find("checked%s*[:=]%s*false")
            or lower:find("done%s*[:=]%s*false")
            or lower:find("complete%s*[:=]%s*false") then
        return "unchecked"
    end
    return nil
end

local function clipboardFlagState(t)
    if type(t) ~= "table" then return nil end
    for k, v in pairs(t) do
        local key = tostring(k):lower()
        if key:find("checked", 1, true)
                or key:find("done", 1, true)
                or key:find("complete", 1, true) then
            if v == false then return "unchecked" end
            if v == true then return "checked" end
        end
    end
    return nil
end

local function collectFlatStrings(value, out, depth)
    if depth > 8 then return end
    if type(value) == "string" then
        local text = cleanClipboardText(value)
        if text ~= "" then out[#out + 1] = text end
    elseif type(value) == "table" then
        for _, child in pairs(value) do
            collectFlatStrings(child, out, depth + 1)
        end
    end
end

local function collectClipboardLines(value, out, depth, inheritedState)
    if depth > 8 then return end
    local tv = type(value)
    if tv == "string" then
        local text = cleanClipboardText(value)
        if text ~= "" then
            out[#out + 1] = {
                text = text,
                state = clipboardLineState(text) or inheritedState,
            }
        end
    elseif tv == "table" then
        local state = clipboardFlagState(value) or inheritedState
        if state == "unchecked" then
            local flat = {}
            collectFlatStrings(value, flat, 0)
            if #flat > 0 then
                out[#out + 1] = {
                    text = table.concat(flat, " "),
                    state = "unchecked",
                }
            end
        end
        for _, child in pairs(value) do
            collectClipboardLines(child, out, depth + 1, state)
        end
    end
end

local function writeClipboardDebug(slot, stack, detail, reason)
    local f = io.open(CLIPBOARD_DEBUG_FILE, "w")
    if not f then return end
    f:write("Create clipboard import debug\n")
    f:write("Reason: " .. tostring(reason or "unknown") .. "\n")
    f:write("Slot: " .. tostring(slot or "?") .. "\n")
    f:write("Stack: ")
    if textutils and textutils.serialize then
        f:write(textutils.serialize(stack or {}) .. "\n")
        f:write("Detail:\n" .. textutils.serialize(detail or {}) .. "\n")
    else
        f:write(tostring(stack) .. "\n")
        f:write("Detail: " .. tostring(detail) .. "\n")
    end
    f:close()
end

local function findClipboardDetail()
    if type(vault.getItemDetail) ~= "function" then
        return nil, nil, nil, "vault does not expose getItemDetail"
    end
    for slot, stack in pairs(vault.list()) do
        if stack.name == "create:clipboard" then
            local ok, detail = pcall(vault.getItemDetail, slot)
            if ok then return slot, stack, detail, nil end
            return slot, stack, nil, tostring(detail)
        end
    end
    return nil, nil, nil, "no create:clipboard found in vault"
end

local function clipboardQtyAndLabel(line)
    local text = cleanClipboardText(line)
    text = text:gsub("%[[xX ]%]", " ")
    text = text:gsub("%([xX ]%)", " ")
    text = text:gsub(CLIPBOARD_BOX_EMPTY, " ")
    text = text:gsub(CLIPBOARD_BOX_CHECKED, " ")
    text = text:gsub(CLIPBOARD_CHECK_MARK, " ")
    text = text:gsub(CLIPBOARD_HEAVY_CHECK_MARK, " ")
    text = text:gsub("^%s*[%-%*]+%s*", "")
    text = cleanClipboardText(text)

    local have, need = text:match("(%d+)%s*/%s*(%d+)")
    if have and need and tonumber(need) and tonumber(have)
            and tonumber(need) > tonumber(have) then
        local label = text:gsub("%d+%s*/%s*%d+", " ")
        return tonumber(need) - tonumber(have), cleanClipboardText(label)
    end

    local n, label = text:match("^x(%d+)%s+(.+)$")
    if n and label then return tonumber(n), cleanClipboardText(label) end

    n, label = text:match("^(%d+)%s*[xX]%s+(.+)$")
    if n and label then return tonumber(n), cleanClipboardText(label) end

    label, n = text:match("^(.-)%s+[xX]%s*(%d+)%s*$")
    if n and label then return tonumber(n), cleanClipboardText(label) end

    label, n = text:match("^(.-)%s+%((%d+)%)%s*$")
    if n and label then return tonumber(n), cleanClipboardText(label) end

    return 1, text
end

local function resolveClipboardRecipe(label)
    local norm = normalizeClipboardLabel(label)
    ...
    for _, rec in ipairs(allRecipes) do
        local labels = {
            rec.name,
            rec.output,
            tostring(rec.output or ""):match(":(.+)$"),
        }
        for _, candidate in ipairs(labels) do
            local cn = normalizeClipboardLabel(candidate)
            if cn == norm then return rec end
            if #cn >= 3 and #cn > bestLen
                    and (norm:find(cn, 1, true) or cn:find(norm, 1, true)) then
                best = rec
                bestLen = #cn
            end
        end
...
local function importClipboardQueue()
    local slot, stack, detail, err = findClipboardDetail()
    if not detail then
        writeClipboardDebug(slot, stack, detail, err)
        ui.status = "Clipboard import failed: " .. tostring(err)
        return
    end

    local lines = {}
    collectClipboardLines(detail, lines, 0, nil)

    local hasUnchecked = false
    for _, line in ipairs(lines) do
        if line.state == "unchecked" then hasUnchecked = true; break end
    end
    if not hasUnchecked then
        writeClipboardDebug(slot, stack, detail, "no unchecked clipboard entries found")
        ui.status = "Clipboard readable, but no unchecked entries found; wrote debug file"
        return
    end

    local added, matched, unmatched = 0, 0, 0
    local requested = {}
    for _, line in ipairs(lines) do
        if line.state == "unchecked" then
            local qty, label = clipboardQtyAndLabel(line.text)
            local rec = resolveClipboardRecipe(label)
            if rec and qty and qty > 0 then
                local key = itemKey(rec.output)
                requested[key] = requested[key] or { rec = rec, qty = 0 }
                requested[key].qty = requested[key].qty + qty
                matched = matched + 1
            else
    ...
    for _, req in pairs(requested) do
        local plan = buildCraftPlan(req.rec.output, req.qty)
        if #plan > 0 then
            for _, job in ipairs(plan) do
                queue[#queue + 1] = {
                    rec = job.rec,
    ...
    if added > 0 then
        ui.status = ("Clipboard queued %d job(s) from %d item line(s)%s"):format(
            added, matched, unmatched > 0 and ("; " .. unmatched .. " unmatched") or "")
        tryDispatchNext()
    else
        writeClipboardDebug(slot, stack, detail, "unchecked entries did not match recipes")
        ui.status = "Clipboard had unchecked entries, but none matched recipes; wrote debug"
    end
end
...
local function drawUI()
    ...
    elseif ui.tab == "craft" then
        if ui.searchMode then
            ...
            hint = "Tap recipe=select  |  Tap again=detail  |  L=import clipboard"
        end
    else
...
while true do
    ...
    elseif ev[1] == "key" and ev[2] == keys.l then
        importClipboardQueue()
        drawUI()
